...
const clickOnCookie = document.querySelector("#fortune-cookie");
...
let randomPhrase = document.querySelector("#phraseRandom")
let generateRandomPhrases

clickOnCookie.addEventListener('click', generatePhrases)
document.addEventListener('keydown', pressEnterKey)

function generatePhrases(){
    toggleBox()

    generateRandomPhrases = Math.floor(Math.random()*(phrases.length));

    randomPhrase.innerText = phrases[generateRandomPhrases]
...
function pressEnterKey(e){
    if(e.key == 'Enter' && box1.classList.contains('hide')){
        toggleScreen()
    }
}